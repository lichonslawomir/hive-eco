...
public sealed class Hive : AuditableAggregateRoot<int, string>, ISynchronizableEntity
{
    ...
    public void Update(string name, string uniqueKey)
    {
        Name = name;
        UniqueKey = uniqueKey;
    }

    public void Update(string name, int audioSensorSampleRate, int audioSensorChannels, int audioSensorBitsPerSample)
    {
        Name = name;
