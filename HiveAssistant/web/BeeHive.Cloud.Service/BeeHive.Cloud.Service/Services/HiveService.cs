...
namespace BeeHive.Cloud.Service.Services;

public sealed class HiveService(IHiveRepository hiveRepository,
    ...
    ITimeAggregateSeriesDataRepository timeAggregateSeriesDataRepository) : IHiveService
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        var lasData = await timeSeriesDataRepository.GetFirstOrDefaultAsync(spec, cancellationToken);
        return lasData.exists ? lasData.dto : null;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var item in linerData)
        {
            ...
            list[^1].MaxValue[mapIdArray[valIdx]] = item.MaxValue.ToNullable();
            list[^1].MinValue[mapIdArray[valIdx]] = item.MinValue.ToNullable();
            list[^1].AvgValue[mapIdArray[valIdx]] = item.AvgValue.ToNullable();
            list[^1].MedValue[mapIdArray[valIdx]] = item.MedValue.ToNullable();
            ++valIdx;
        }
