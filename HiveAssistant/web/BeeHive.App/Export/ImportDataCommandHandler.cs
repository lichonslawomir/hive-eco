    ...
    public async Task HandleCommand(ImportDataCommand cmd, CancellationToken cancellationToken)
    {
        if (cmd.Hives is not null)
        {
            foreach (var model in cmd.Hives)
            {
                ...
                if (!newHive)
                {
                    hive.Update(model.Hive.Name, model.Hive.AudioSensorSampleRate, model.Hive.AudioSensorChannels, model.Hive.AudioSensorBitsPerSample);
                }

                var (state, _) = await GetExportState(Domain.BeeGardens.ExportEntity.Hive, beeGarden, newBeeGarden, cancellationToken);
                state.Update(model.CreatedOrUpdatedDate);
                ...
                foreach (var stats in model.Datas)
                {
                    ...
                    if (stats.LastValueTimestamp.HasValue && stats.LastValue.HasValue)
                    {
                        ...
                        if (seriesData is null)
                            _beeHiveDbContext.TimeSeriesData.Add(timeSeries.AddData(stats.LastValueTimestamp.Value, stats.LastValue.Value));
                        else
                    ...
                    {
                        timeAggregateSeriesData = aggregateSeries.CreateData(stats.Timestamp,
                            stats.Count,
                            stats.MaxValue,
                            stats.MinValue,
                            stats.AvgValue,
                    ...
                    state.Update(stats.CreatedOrUpdatedDate);
