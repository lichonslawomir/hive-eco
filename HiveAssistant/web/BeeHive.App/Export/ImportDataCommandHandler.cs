    ...
    public async Task HandleCommand(ImportDataCommand cmd, CancellationToken cancellationToken)
    {
        if (cmd.Hives is not null)
        {
            foreach (var model in cmd.Hives)
            {
                ...
                if (!newHive)
                {
                    hive.Update(model.Hive.Name,
                        model.Hive.ComPort,
                        model.Hive.GraphColor,
                        model.Hive.SerialBound,
                        model.Hive.AudioSensorSampleRate,
                        model.Hive.AudioSensorChannels,
                        model.Hive.AudioSensorBitsPerSample);
                }
